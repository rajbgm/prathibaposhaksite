...
function get_amount($don_cause, $don_category)
{
	...
	if ($don_cause == 'sponsor-device') {
		...
		if ($don_cause == 'Sponsor-Students-Internet') {
		if ($don_category == 'Indian')
			return "3500";
		else
			return "45";
		...
		if ($don_cause == 'Contribute-to-Teacher-Salary') {
		if ($don_category == 'Indian')
			return "";
		else
			return "";
	}
	if ($don_cause == 'sponsor-data-meal') {
		if ($don_category == 'Indian')
			return "1200";
		else
			return "15";
	}
	if ($don_cause == 'sponsor-data-schoolbag') {
		if ($don_category == 'Indian')
			return "350";
		else
			return "5";
...
function get_cause($don_cause)
{
	...
	if ($don_cause == 'Sponsor-Students-Internet') {
		return 'Internet Data for one year';
	}
	if ($don_cause == 'sponsor-data') {
		return 'Internet Data for 2 years';
	}
	if ($don_cause == 'sponsor-data-meal') {
		return 'Meal for 20 students on Pratibha Poshak Selection Test Day';
	}
	if ($don_cause == 'sponsor-data-schoolbag') {
		return 'A school bag (backpack) for a Pratibha Poshak student';
	}	
	else {
		return "";
	}
...
<body>
	...
	<section class="space-ptb bg">
		<form id="donationForm1" name="donate_step2" action="donate_step3.php" method="post">
			<div class="container">
				<div class="row">
					<div class="col-sm-12">
				...
				<div class="row">
					<div class="form-container">
						...
						<div class="row pt-3 pb-3">
							<div class="col-md-3">
								<label class="labelForm">Donor Name (*)</label>
							</div>
							<div class="col-md-9">
								<input type="text" id="billing_name" name="billing_name" value=""
									class="form-control-donate" />
								<span id="billing_nameError" style="color: red;"></span>
							</div>
						</div>
						<div class="row pb-3">
							<div class="col-md-3">
							...
							<div class="col-md-9">
								<input type="text" id="billing_tel" name="billing_tel" value=""
									class="form-control-donate" />
						...
						<div class="row pb-3">
							<div class="col-md-3">
								<label class="labelForm">Currency (*)</label>
							</div>
							<div class="col-md-9">
						...
						<div class="row pb-3">
							<div class="col-md-3">
								<label class="labelForm">Donation for (*)</label>
							</div>
							<div class="col-md-9">
								<input type="text" name="merchant_param2" value="<?php echo get_cause($donation); ?>"
									class="form-control-donate" />
							</div>
						</div>
						<div class="row pb-3" id="panField" style="display: none;">
							<div class="col-md-3">
								<label class="labelForm">Your PAN (*)</label>
							</div>
							<div class="col-md-9">
								<input type="text" id="merchant_param1" name="merchant_param1" value=""
									class="form-control-donate" />
						...
						<div class="row pb-3">
							<div class="col-md-3">
								<label class="labelForm">PIN/ZIP Code</label>
							</div>
							<div class="col-md-9">
						...
						<div class="row pb-3">
							...
							<div class="col-md-9">
								<input type="text" name="billing_country" value="<?php echo ($donor_category == 'Indian' ? 'India' : 'United States');?>" class="form-control-donate" />
							</div>
					...
					<div class="form-container">
						...
						'<div class="row">
							...
							<div class="col-sm-12 text-center">
								...
								<br>								
									<span id="payModeError" style="color: red;"></span>
								</div>
						</div>'; }
						?>
						...
						<div class="row pb-3">
							...
							<div class="col-sm-12 text-center">
								<input TYPE="submit" value="DONATE" class="btn btn-sm rcfBtn btn-white">
							</div>
						</div>
					</div>
				</div>
		...
		<script>
			...
			document.getElementById('donationForm1').addEventListener('submit', function (event) {
				...
				document.getElementById('payModeError').textContent = '';
				...
				var mobileValue = document.getElementById('billing_tel').value.trim();
				var mobilePattern = /^\d{10}$/;
				if (!mobilePattern.test(mobileValue)) {
					document.getElementById('billing_telError').textContent = 'Please enter a valid 10-digit Phone number.';
					event.preventDefault(); // Prevent form submission
				}
				...
				var panValue = document.getElementById('merchant_param1').value.trim();
				var panPattern = /^[A-Z]{5}[0-9]{4}[A-Z]$/;
				if (!panPattern.test(panValue)) {
					document.getElementById('panError').textContent = 'Please enter a valid PAN number (e.g., ABCDE1234F).';
					event.preventDefault(); // Prevent form submission
				}

				<?php if ($donor_category == 'Indian') { ?>				
				var payModeElements = document.getElementsByName('paymode');
				var selectedpayModeType = false;
				for (var i = 0; i < payModeElements.length; i++) {
					if (payModeElements[i].checked) {
						selectedpayModeType = true;
						break;
					}
				}	
				if (!selectedpayModeType) {			
					document.getElementById('payModeError').textContent = 'Please select a Payment Mode';
					event.preventDefault(); // Prevent form submission
				}
				<?php } ?>
			});

			var nationality = '<?php echo $_POST['donorcategory'] ? $_POST['donorcategory'] : ''; ?>';
			var panField = document.getElementById('panField');			
			if (nationality === 'Indian') {
				panField.style.display = 'flex';
